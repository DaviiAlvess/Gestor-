(function (root) {
  ...
  function installments(amount, count, date) {
    if (!Number.isFinite(amount) || amount <= 0 || !Number.isInteger(count) || count < 1 || count > 360 || !validDate(date)) throw new Error('Confira o valor, a data e a quantidade de parcelas.');
  ...
  function income(amount, growth, months) {
    ...
    if (!Number.isFinite(total)) throw new Error('Os valores excedem o limite da simulação.');
